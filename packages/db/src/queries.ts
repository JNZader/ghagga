...
export async function getDelegatedCiPolicy(db: Database, repoId: number) {
  const [repo] = await db
    .select({ delegatedCiPolicy: repositories.delegatedCiPolicy })
    .from(repositories)
    .where(eq(repositories.id, repoId))
    .limit(1);
  return repo?.delegatedCiPolicy ?? null;
}

export async function updateDelegatedCiPolicy(
  db: Database,
  repoId: number,
  policy: DbDelegatedCiPolicy | null,
) {
  await db
    .update(repositories)
    .set({ delegatedCiPolicy: policy, updatedAt: new Date() })
    .where(eq(repositories.id, repoId));
}

// ─── Delegated CI Runs ──────────────────────────────────────────

export async function createDelegatedCiRun(
  db: Database,
  data: {
    repositoryId: number;
    prNumber?: number;
  ...
  const [result] = await db.insert(delegatedCiRuns).values(data).returning();
  // biome-ignore lint/style/noNonNullAssertion: drizzle .returning() always returns for insert/update
  return result!;
}

export async function updateDelegatedCiRunState(
  db: Database,
  runId: number,
  updates: {
    state: string;
  ...
  const [result] = await db
    ...
    .set({ ...updates, updatedAt: new Date() })
    .where(eq(delegatedCiRuns.id, runId))
    .returning();
  // biome-ignore lint/style/noNonNullAssertion: drizzle .returning() always returns for insert/update
  return result!;
}

export async function getDelegatedCiRun(db: Database, runId: number) {
  const [row] = await db
    .select()
    .from(delegatedCiRuns)
    .where(eq(delegatedCiRuns.id, runId))
    .limit(1);
  return row ?? null;
}

export async function getDelegatedCiRunByCallbackId(db: Database, callbackId: string) {
  const [row] = await db
    .select()
    .from(delegatedCiRuns)
...
export async function getDelegatedCiRunsByRepoId(
  ...
  return db
    .select()
    .from(delegatedCiRuns)
    .where(eq(delegatedCiRuns.repositoryId, repositoryId))
    .orderBy(desc(delegatedCiRuns.createdAt))
    .limit(limit)
    .offset(offset);
}

export async function countDelegatedCiRunsByRepoId(db: Database, repositoryId: number) {
  const [result] = await db
