 ...
 *
 * RESULT (this session): only TypeScript and JavaScript resolve. Python,
 * Rust, Java, and Go all fail to resolve — each uses an import-specifier
 * style resolveImportPath was never designed to handle (module/package
 * paths or a `.`-prefixed convention that isn't a joinable relative path).
 */
...
describe('per-language graph resolution (task 4.5)', () => {
  ...
  it('Python: relative import does NOT resolve seed -> dependent (resolveImportPath cannot map `.seed` to `seed.py`)', () => {
    // `from .seed import seed` extracts source `.seed`. resolveImportPath treats
    // any `.`-prefixed specifier as a joinable relative PATH segment (not Python's
    // package-relative-module convention), so it joins to `pkg/.seed` and tries
    // extensions (`pkg/.seed.py`) — never matching the real `pkg/seed.py` node.
    // Empirically confirmed: builder.ts's resolver is TS/JS-import-style specific.
    const files = new Map<string, string>([
    ...
    expect(result.dependents).not.toContain('pkg/dependent.py');
  });
