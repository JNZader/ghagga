...
import { type CriticFn, type FixerFn, formatQAResult, runQALoop } from './adversarial-qa.js';
...
describe('runQALoop', () => {
  it('passes on first round when score is high', async () => {
    const result = await runQALoop('good code', makeCritic([90]), makeFixer(0));
  ...
  it('iterates until score passes', async () => {
    const result = await runQALoop('bad code', makeCritic([40, 60, 85]), makeFixer(1));
    assert(result.passed);
  ...
  it('stops at max rounds', async () => {
    const result = await runQALoop('terrible code', makeCritic([20, 30, 40]), makeFixer(1), {
      maxRounds: 3,
      passThreshold: 80,
  ...
  it('tracks total issues found and resolved', async () => {
    const result = await runQALoop('code', makeCritic([50, 85]), makeFixer(1));
  ...
  it('fixer result is null on final round', async () => {
    const result = await runQALoop('code', makeCritic([90]), makeFixer(1));
...
describe('formatQAResult', () => {
  ...
  it('shows FAILED for failing result', async () => {
    const result = await runQALoop('code', makeCritic([30]), makeFixer(0), {
      maxRounds: 1,
      passThreshold: 80,
      stopOnNoProgress: false,
    });
...
function assert(condition: boolean, _msg?: string) {
