...
export const AISVS_CHECKS: AISVSCheck[] = [
  // C11 — Prompt Injection Prevention
  {
    ...
    recommendation:
      'Never interpolate user input into system prompts. Use a separate user message role.',
  },
  {
    id: 'AISVS-11.2',
    category: 'prompt-injection',
    ...
    recommendation:
      'Sanitize user input before embedding in prompts. Use parameterized prompt templates.',
  },
  {
    id: 'AISVS-11.3',
    category: 'prompt-injection',
    ...
    pattern:
      /(?:generateText|chat\.completions|anthropic\.messages).*(?:req\.body|req\.query|req\.params)/i,
  ...
  // C12 — Output Validation
  {
    ...
    pattern:
      /(?:eval|exec|execSync|spawn|fork)\s*\(\s*(?:result|response|output|completion|text)\b/i,
    recommendation:
      'Never execute LLM output as code. Parse and validate structured output instead.',
  },
  {
    id: 'AISVS-12.2',
    category: 'output-validation',
    ...
    pattern:
      /(?:innerHTML|dangerouslySetInnerHTML|v-html)\s*[=:]\s*.*(?:result|response|completion|generated)/i,
  ...
  },
  {
    ...
    pattern:
      /(?:\.query|\.execute|\.run)\s*\(\s*[`'"]\s*(?:SELECT|INSERT|UPDATE|DELETE|CREATE).*\$\{.*(?:result|response|completion|output)/i,
    recommendation:
      'Use parameterized queries. Never interpolate LLM output into database operations.',
  },
  ...
  // C10 — MCP Security
  {
    ...
    recommendation:
      'MCP tools should not execute arbitrary shell commands. Use allowlists and sandboxing.',
  },
  {
    id: 'AISVS-10.2',
    category: 'mcp-security',
    ...
    pattern:
      /(?:tool|handler).*(?:args|params|input)\s*(?:\.|\[)(?!.*(?:validate|schema|zod|parse))/i,
  ...
  // Data leakage
  {
    ...
    pattern:
      /(?:log|console|print).*(?:prompt|response|completion).*(?:email|phone|ssn|address|name)/i,
...
export function buildAISVSReport(allFindings: AISVSFinding[], filesScanned: number): AISVSReport {
  const checksRun = AISVS_CHECKS.length;
  const uniqueChecksTriggered = new Set(allFindings.map((f) => f.checkId)).size;
  const passRate =
    checksRun > 0 ? Math.round(((checksRun - uniqueChecksTriggered) / checksRun) * 100) : 100;
