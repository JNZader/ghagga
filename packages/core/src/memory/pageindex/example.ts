...
import { ProjectPageIndexService, PageDirection } from './index.js';
...
export async function exampleReviewWorkflow(
  ...
  if (compactionCheck.shouldCompact) {
    console.log(`⚠️ Project memory ${compactionCheck.currentTokens} tokens exceeds safe limit for ${modelMaxTokens} model`);
...
export function exampleContextWindow(
  ...
  console.log(`📄 Context window: ${context.previousPages.length} before, ${context.nextPages.length} after`);
  console.log(`📊 Total in window: ${context.totalInContext} pages (${context.totalTokens} tokens)`);
