...
export class SqliteMemoryStorage implements MemoryStorage {
  ...
  async searchObservations(
    ...
    while (stmt.step()) {
      ...
      rows.push({
        ...
        strength,
        // Surface keyword relevance (saturating bm25 → [0,1]) for observability.
  ...
  private async _hybridSearch(
    ...
    for (const { candidate, strength, finalScore } of scored) {
      ...
      results.push({
        ...
        strength,
        // Hybrid finalScore (0.7*cosine + 0.3*normalizedBm25) is already in [0,1].
