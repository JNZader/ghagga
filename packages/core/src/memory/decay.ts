 ...
 *   - rank → ∞      → → 0.0
 *
 * INTENDED FOR A FUTURE Postgres adapter — one that would return rows already
 * ordered by `ts_rank` DESC without exposing the raw rank value, so a 0-based
 * position becomes a stable, bounded proxy COMPARABLE to the SQLite bm25 mapping
 * for a shared `relevanceScore` field (both saturate, both monotonic). NOTE: no
 * such Postgres adapter exists in this package yet, so this function currently
 * has NO callers — it is scaffolding, NOT live. Dedup relevance is not wired on
 * Postgres today; do not read this as evidence that it is.
 *
