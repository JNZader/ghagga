...
describe("deriveRules", () => {
	...
	it("creates no rule when neither threshold is met", () => {
		...
		const feedback = [
			...makeRejections("performance", 3),
			...makeAcceptances("performance", 3),
		];
		const rules = deriveRules(feedback);
...
describe("formatRulesForPrompt", () => {
	...
	it("formats suppress rules with rejection rate", () => {
		const rules: ImprovementRule[] = [
			{ pattern: "category:style", category: "style", action: "suppress", confidence: 0.8, sampleCount: 10 },
		];
	...
	it("formats boost rules with acceptance rate", () => {
		const rules: ImprovementRule[] = [
			{ pattern: "category:security", category: "security", action: "boost_priority", confidence: 0.9, sampleCount: 15 },
		];
		const output = formatRulesForPrompt(rules);
	...
	it("includes header section", () => {
		const rules: ImprovementRule[] = [
			{ pattern: "category:style", category: "style", action: "suppress", confidence: 0.75, sampleCount: 8 },
		];
		const output = formatRulesForPrompt(rules);
