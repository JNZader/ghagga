...
| **Privacy stripping** | 24 regex patterns remove secrets before storing to memory |
...
See [Memory System -- Privacy Stripping](memory-system.md#privacy-stripping) for the full list of 24 patterns that are stripped before storing observations.
...
- Privacy stripping covers all 24 secret patterns
...
5. **Limit GitHub App permissions** — Only request `pull_requests: write`, `actions: write`, `issues: write`, and `metadata: read` (auto). `Contents: Write` is needed to inject the inline workflow file into the target repo at `.github/workflows/ghagga.yml`. `Issues: Write` is needed for the issue-triage feature (read the issue + its comments, post the approved draft comment). The `administration` permission is not needed. `Secrets: Read & Write` is **not** required — GHAGGA no longer fans out repository secrets to a separate runner repo.

   > **Re-consent warning**: Adding the `issues` permission triggers a GitHub-mandated **re-consent** prompt for every existing installation — admins must approve the new scope before the App keeps working on their repos. Bundle this change with the pre-launch deploy consent (the issue-triage feature is release-blocked on server deploy + the PRE-LAUNCH 🔐 list anyway) so installations are only re-prompted once.
