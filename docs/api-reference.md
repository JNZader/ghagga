...
> **Deprecated**: `enableSemgrep`, `enableTrivy`, and `enableCpd` are deprecated since v2.5.0. Use `enabledTools`/`disabledTools` instead to control individual tools from the 17-tool plugin registry. The legacy boolean fields still work but will be removed in a future version.
