...
- **Run-count cap**: per-marker-directory indexer runs are capped at **25** by default. Runs are sorted `stable` → `heavy` → `experimental` maturity, then root-first by depth, BEFORE capping — so cheap, reliable indexers never get crowded out by expensive/immature ones, and a pathological monorepo degrades predictably rather than hanging. Beyond the cap, a warning names exactly which marker directories were skipped.
