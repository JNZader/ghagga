...
// Mocked in vi.mock('./utils.js') above — imported here to assert what was logged.
...
describe('POST /api/issue-drafts/:id/approve', () => {
  ...
  it('PRE-POST failure: getInstallationToken throws → claim released, no comment posted', async () => {
    // The token exchange runs INSIDE the release-on-failure boundary. Since it
    // fails BEFORE postComment, nothing is live on GitHub → the claim is reverted
    // (APPROVED → DRAFT) so a human can retry, and postComment is never called.
    mockGetIssueDraftById.mockResolvedValue(makeDraft());
    ownRepo7();
    mockGetRepositoryById.mockResolvedValue({ id: 7, fullName: 'acme/app', installationId: 100 });
    mockGetInstallationById.mockResolvedValue({ id: 100, githubInstallationId: 555000 });
    mockClaimIssueDraftForPosting.mockResolvedValue(makeDraft({ status: 'APPROVED' }));
    mockGetInstallationToken.mockRejectedValue(new Error('token exchange 500'));
    mockReleaseIssueDraftClaim.mockResolvedValue(makeDraft({ status: 'DRAFT' }));
    const app = createApp();
    ...
    expect(res.status).toBe(502);
    // claim released so the draft is retryable; nothing posted, nothing recorded
    expect(mockReleaseIssueDraftClaim).toHaveBeenCalledWith(mockDb, 9);
    expect(mockPostComment).not.toHaveBeenCalled();
    expect(mockMarkIssueDraftPosted).not.toHaveBeenCalled();
  });

  it('POST-SUCCEEDED failure: markPosted throws AFTER a live post → claim NOT released, error carries comment id', async () => {
    // The comment is ALREADY live on GitHub. Recording POSTED then fails on every
    // retry. Releasing would let a retry double-post, so the claim is LEFT (draft
    // stays APPROVED), postComment is called exactly ONCE, and the failure is
    // logged + surfaced with the comment id for reconciliation (non-retryable).
    mockGetIssueDraftById.mockResolvedValue(makeDraft());
    wireApprovePrereqs();
    mockClaimIssueDraftForPosting.mockResolvedValue(makeDraft({ status: 'APPROVED' }));
    mockPostComment.mockResolvedValue({ id: 987654321 });
    mockMarkIssueDraftPosted.mockRejectedValue(new Error('DB write timeout'));
    const app = createApp();

    const res = await app.request('/api/issue-drafts/9/approve', { method: 'POST' });
    const json = (await res.json()) as { error: string; commentId: number };

    expect(res.status).toBe(500);
    expect(json.error).toBe('POST_RECORDING_FAILED');
    // the live comment id is surfaced so the stuck-APPROVED draft is reconcilable
    expect(json.commentId).toBe(987654321);
    // NEVER released — releasing would allow a second comment on retry
    expect(mockReleaseIssueDraftClaim).not.toHaveBeenCalled();
    // posted exactly once (no double post), and the mark was retried (bounded)
    expect(mockPostComment).toHaveBeenCalledTimes(1);
    expect(mockMarkIssueDraftPosted).toHaveBeenCalledTimes(3);
    // error logged with BOTH the draft id and the live comment id
    expect(logger.error).toHaveBeenCalledWith(
      expect.objectContaining({ id: 9, commentId: 987654321 }),
      expect.any(String),
    );
  });
