...
function DraftDetail({ draft, onClose }: DraftDetailProps) {
  ...
  function handleSave() {
    editDraft.mutate(
      { id: draft.id, body },
      {
        onSuccess: () => addToast({ message: 'Draft saved', type: 'success' }),
...
export function IssueTriage() {
  ...
  return (
    <div>
      ...
      {selected && <DraftDetail draft={selected} onClose={() => setSelected(null)} />}
    </div>
  );
}
