...
describe('IssueTriage detail', () => {
  ...
  it('after a successful save the Save button disables and a second click does NOT re-PATCH (REL-001)', () => {
    // The edit resolves successfully → onSuccess fires synchronously, syncing the
    // selected draft's body so bodyDirty clears.
    ...
    // Body now equals the saved value → not dirty → the button is disabled.
    expect(saveButton).toBeDisabled();
    // A second click must NOT fire another PATCH (the stale-dirty bug).
    fireEvent.click(saveButton);
    expect(mockEditMutate).toHaveBeenCalledTimes(1);
  });
