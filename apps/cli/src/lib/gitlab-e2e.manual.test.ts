 ...
 *
 * It will: resolve the numeric project id from the path, post a marker-tagged
 * summary note, assert that note is live with MARKER.html, then upsert AGAIN.
 * Live GitLab uses update-in-place (FORGE-UPSERT-005): the second call MUST
 * return the SAME note id, deletedNativeIds empty, exactly one marker note,
 * and the body of that note MUST be the second payload. An `afterAll` ATTEMPTS
 * to leave zero leftover notes (best-effort).
 *
...
describe('GitLab MR summary post-back — LIVE E2E (manual gate, R-GITLAB)', () => {
  ...
  it.skipIf(!PAT)(
    ...
    async () => {
      ...
      const body1 = `${MARKER.html}\n## GHAGGA E2E\nfirst ${new Date().toISOString()}`;
      const body2 = `${MARKER.html}\n## GHAGGA E2E\nsecond ${new Date().toISOString()}`;

      // First upsert: posts a fresh note.
      const first = await postSummaryComment(adapter, ref, body1, MARKER);
      ...
      // Second upsert: update-in-place (FORGE-UPSERT-005). Same note id, no delete.
      const second = await postSummaryComment(adapter, ref, body2, MARKER);
      expect(second.createdNativeId).toBe(first.createdNativeId);
      expect(second.deletedNativeIds).toEqual([]);
