...
> - política del `security` audit (¿fallar en CRITICAL?) — hoy `pnpm audit --prod --audit-level=high` con `continue-on-error: true` (advisory, `ci.yml:67-74`); pre-existente repo-wide de CVEs de prod. PR #261 bajó el CVE de undici (TLS cert bypass). Decisión pendiente: ¿hacerlo blocking?
